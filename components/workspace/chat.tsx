  ...
  return (
    <section className="w-full h-full bg-canvassoft flex flex-col overflow-hidden">
      ...
      <div ref={threadRef} className="flex-1 overflow-y-auto p-[18px] flex flex-col gap-4">
        {messages.map((m, i) => m.role === "u" ? <UserMsg key={i} refs={m.refs} attachments={m.attachments}>{m.text}</UserMsg> : <AgentMsg key={i} m={m} />)}
      </div>
      <div className="p-3.5 pt-2.5">
        <div className="bg-surface border border-hairline rounded-2xl px-3 py-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          ...
          <textarea rows={1} value={input} onChange={(e) => setInput(e.target.value)}
            ...
            placeholder={busy ? "Gampex 正在处理…" : "继续聊，或上传参考…"} className="w-full resize-none outline-none bg-transparent text-[13.5px] leading-snug text-ink placeholder:text-faint" />
